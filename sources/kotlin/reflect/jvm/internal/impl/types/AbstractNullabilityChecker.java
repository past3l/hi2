        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(pop)) {
                ...
                if (none == null) {
                    ...
                    while (it.hasNext()) {
                        SimpleTypeMarker mo1910transformType = none.mo1910transformType(typeCheckerState, it.next());
                        if ((typeSystemContext.isClassType(mo1910transformType) && !typeSystemContext.isMarkedNullable(mo1910transformType)) || typeSystemContext.isDefinitelyNotNullType(mo1910transformType)) {
                            typeCheckerState.clear();
                            return true;
                        }
                        supertypesDeque.add(mo1910transformType);
                    }
                }
            }
        }
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(pop)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(pop)).iterator();
                    while (it.hasNext()) {
                        SimpleTypeMarker mo1910transformType = supertypesPolicy.mo1910transformType(state, it.next());
                        if (INSTANCE.isApplicableAsEndNode(state, mo1910transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(mo1910transformType);
