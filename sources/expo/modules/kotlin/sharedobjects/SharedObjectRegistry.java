...
public final class SharedObjectRegistry {
    ...
    public SharedObjectRegistry(AppContext appContext) {
        ...
        this.currentId = SharedObjectId.m576constructorimpl(1);
        this.pairs = new LinkedHashMap();
    }

    /* renamed from: pullNextId-HSeVr_g, reason: not valid java name */
    private final int m584pullNextIdHSeVr_g() {
        int i;
        synchronized (this) {
            i = this.currentId;
            this.currentId = SharedObjectId.m576constructorimpl(i + 1);
        }
        return i;
    }

    /* renamed from: add-5WKnsLU$expo_modules_core_release, reason: not valid java name */
    public final int m585add5WKnsLU$expo_modules_core_release(SharedObject r8, JavaScriptObject js) {
        Intrinsics.checkNotNullParameter(r8, "native");
        Intrinsics.checkNotNullParameter(js, "js");
        int m584pullNextIdHSeVr_g = m584pullNextIdHSeVr_g();
        r8.m574setSharedObjectIdkyJHjyY$expo_modules_core_release(m584pullNextIdHSeVr_g);
        JavaScriptObject.defineProperty$default(js, SharedObjectRegistryKt.sharedObjectIdPropertyName, m584pullNextIdHSeVr_g, (List) null, 4, (Object) null);
        AppContext appContext = this.appContextHolder.get();
        if (appContext == null) {
            throw new Exceptions.AppContextLost();
        }
        appContext.getJsiInterop$expo_modules_core_release().setNativeStateForSharedObject(m584pullNextIdHSeVr_g, js);
        JavaScriptWeakObject createWeak = js.createWeak();
        synchronized (this) {
            this.pairs.put(SharedObjectId.m575boximpl(m584pullNextIdHSeVr_g), TuplesKt.to(r8, createWeak));
        ...
        return m584pullNextIdHSeVr_g;
    }

    /* renamed from: delete-kyJHjyY$expo_modules_core_release, reason: not valid java name */
    public final void m586deletekyJHjyY$expo_modules_core_release(int id) {
        Pair<SharedObject, JavaScriptWeakObject> remove;
        synchronized (this) {
            remove = this.pairs.remove(SharedObjectId.m575boximpl(id));
        }
        if (remove != null) {
            SharedObject component1 = remove.component1();
            component1.m574setSharedObjectIdkyJHjyY$expo_modules_core_release(SharedObjectId.m576constructorimpl(0));
            component1.deallocate();
        }
    }

    /* renamed from: toNativeObject-kyJHjyY$expo_modules_core_release, reason: not valid java name */
    public final SharedObject m587toNativeObjectkyJHjyY$expo_modules_core_release(int id) {
        SharedObject first;
        synchronized (this) {
            Pair<SharedObject, JavaScriptWeakObject> pair = this.pairs.get(SharedObjectId.m575boximpl(id));
    ...
    public final SharedObject toNativeObject$expo_modules_core_release(JavaScriptObject js) {
        ...
        Pair<SharedObject, JavaScriptWeakObject> pair = this.pairs.get(SharedObjectId.m575boximpl(SharedObjectId.m576constructorimpl(js.getProperty(SharedObjectRegistryKt.sharedObjectIdPropertyName).getInt())));
        if (pair != null) {
    ...
    public final JavaScriptObject toJavaScriptObject$expo_modules_core_release(SharedObject r2) {
        ...
        synchronized (this) {
            Pair<SharedObject, JavaScriptWeakObject> pair = this.pairs.get(SharedObjectId.m575boximpl(r2.getSharedObjectId()));
