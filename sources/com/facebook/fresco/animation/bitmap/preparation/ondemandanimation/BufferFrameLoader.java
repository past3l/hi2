...
public final class BufferFrameLoader implements FrameLoader {
    ...
    public FrameResult getFrame(int frameNumber, int width, int height) {
        ...
        if (bufferFrame != null) {
            ...
            return new FrameResult(bufferFrame.getBitmapRef().mo142clone(), FrameResult.FrameType.SUCCESS);
        }
        loadNextFrames(width, height);
    ...
    private final FrameResult findNearestToRender(int targetFrame) {
        ...
        if (findNearestFrame != null) {
            CloseableReference<Bitmap> mo142clone = findNearestFrame.getBitmap().mo142clone();
            Intrinsics.checkNotNullExpressionValue(mo142clone, "nearestFrame.bitmap.clone()");
            this.lastRenderedFrameNumber = findNearestFrame.getFrameNumber();
            return new FrameResult(mo142clone, FrameResult.FrameType.NEAREST);
        }
