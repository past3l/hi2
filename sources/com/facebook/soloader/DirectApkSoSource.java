    ...
    private void prepare() throws IOException {
        ...
        for (String str2 : this.mDirectApkLdPaths) {
            ...
            if (!TextUtils.isEmpty(str)) {
                ZipFile zipFile = new ZipFile(getApkPathFromLdPath(str2));
                try {
                    ...
                    while (entries.hasMoreElements()) {
                        ...
                        if (nextElement != null && nextElement.getMethod() == 0 && nextElement.getName().startsWith(str) && nextElement.getName().endsWith(".so")) {
                            appendLibsInApkCache(str2, nextElement.getName().substring(str.length() + 1));
                        }
                    }
                    zipFile.close();
                } catch (Throwable th) {
            ...
            while (entries.hasMoreElements()) {
                ...
                if (nextElement != null) {
                    if (nextElement.getName().endsWith("/" + str2)) {
                        buildLibDepsCacheImpl(str, zipFile, nextElement, str2);
                    }
                }
            }
            zipFile.close();
