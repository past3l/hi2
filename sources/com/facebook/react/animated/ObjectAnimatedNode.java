...
class ObjectAnimatedNode extends AnimatedNode {
    ...
    public void collectViewUpdates(String str, JavaOnlyMap javaOnlyMap) {
        ReadableType type = this.mConfig.getType(VALUE_KEY);
        if (type == ReadableType.Map) {
            javaOnlyMap.putMap(str, collectViewUpdatesHelper(this.mConfig.getMap(VALUE_KEY)));
        } else {
            if (type == ReadableType.Array) {
                javaOnlyMap.putArray(str, collectViewUpdatesHelper(this.mConfig.getArray(VALUE_KEY)));
                return;
            }
