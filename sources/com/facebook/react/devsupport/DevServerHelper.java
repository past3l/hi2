...
public class DevServerHelper {
    ...
    public String getInspectorDeviceUrl() {
        return String.format(Locale.US, "http://%s/inspector/device?name=%s&app=%s&device=%s", this.mPackagerConnectionSettings.getServerIp(), Uri.encode(AndroidInfoHelpers.getFriendlyDeviceName()), Uri.encode(this.mPackageName), Uri.encode(getInspectorDeviceId()));
    }
    ...
    public File downloadBundleResourceFromUrlSync(String str, File file) {
        try {
            Response execute = this.mClient.newCall(new Request.Builder().url(createResourceURL(this.mPackagerConnectionSettings.getServerIp(), str)).build()).execute();
            try {
    ...
    public void openDebugger(final ReactContext reactContext, final String str) {
        this.mClient.newCall(new Request.Builder().url(String.format(Locale.US, "http://%s/open-debugger?appId=%s&device=%s", this.mPackagerConnectionSettings.getServerIp(), Uri.encode(this.mPackageName), Uri.encode(getInspectorDeviceId()))).method(ShareTarget.METHOD_POST, RequestBody.create((MediaType) null, "")).build()).enqueue(new Callback() { // from class: com.facebook.react.devsupport.DevServerHelper.6
            @Override // okhttp3.Callback
            public void onResponse(Call call, Response response) {
            }
